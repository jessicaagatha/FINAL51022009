...
public class MenuUtama extends javax.swing.JFrame {
    ...
    private void tiketActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_tiketActionPerformed
        updateTiketButton(); // Panggil metode updateTiketButton saat tombol tiket diklik
    }//GEN-LAST:event_tiketActionPerformed

    private void updateTiketButton() {
        try {
        String query = "SELECT userfilm.id_tiket, userfilm.id_user, userfilm.id_film, userfilm.jumlah_tiket, userfilm.no_kursi, userfilm.total_harga_tiket, " +
                "film.judul AS judul_film, film.genre AS genre_film, film.durasi_film, film.hari_tayang, film.waktu_tayang " +
                "FROM userfilm " +
                "JOIN film ON userfilm.id_film = film.id_film " +
                "WHERE userfilm.id_user = ?";

        Database db = new Database(query);
        db.setString(1, SesiLogUser.getUserId());
        var rs = db.getResultSet();

        StringBuilder tiketInfo = new StringBuilder();
        while (rs.next()) {
            ...
            String hargaTiketFormatted = rs.getString("total_harga_tiket");
            String judulFilm = rs.getString("judul_film");
            String genreFilm = rs.getString("genre_film");
            String durasiFilm = rs.getString("durasi_film");
            String hariTayang = rs.getString("hari_tayang");
            String waktuTayang = rs.getString("waktu_tayang");

            tiketInfo.append("============== INFORMASI TIKET ===============\n");
            tiketInfo.append("ID Tiket: ").append(idTiket).append("\n");
            tiketInfo.append("ID User: ").append(idUser).append("\n");
            tiketInfo.append("Judul Film: ").append(judulFilm).append("\n");
            tiketInfo.append("Genre Film: ").append(genreFilm).append("\n");
            tiketInfo.append("Durasi Film: ").append(durasiFilm).append("\n");
            tiketInfo.append("Hari Tayang: ").append(hariTayang).append("\n");
            tiketInfo.append("Waktu Tayang: ").append(waktuTayang).append("\n");
            tiketInfo.append("Jumlah Tiket: ").append(jumlahTiket).append("\n");
            tiketInfo.append("Nomor Kursi: ").append(nomorKursi).append("\n");
            tiketInfo.append("Harga Tiket: ").append(hargaTiketFormatted).append("\n\n");
        }
        TiketUser tiket = new TiketUser();
        tiket.setTiketText(tiketInfo.toString());
